...
const MainChat = forwardRef(({ onSourceClick }, ref) => {
  ...
  const handleSend = async (text) => {
    ...
    setQuery('');
    setIsLoading(true);
    setStatusText('Searching filings and drafting an answer…');
    ...
    try {
      ...
      setMessages([
        ...newMsgs,
        {
          ...
          scoped_sources: Array.isArray(res.data.scoped_sources) ? res.data.scoped_sources : null,
          auto_scoped: Boolean(res.data.auto_scoped),
        },
      ]);
    } catch (err) {
          ...
          <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
            {messages.map((msg, i) => (
              <div key={i} className={`flex flex-col gap-1.5 ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                           ...
                           code: ({ node, inline, className, children, ...props }) => {
                             ...
                             if (!inline && match && match[1] === 'chart-data') {
                               try {
                                ...
                                const xKey =
                                  allKeys.find(k =>
                                    ['year', 'date', 'name', 'label', 'x', 'period', 'point', 'metric'].includes(
                                      k.toLowerCase(),
                                    ),
                                  ) || allKeys[0];
                                ...
                                if (keys.length === 0) {
                                 ...
                                 return (
                                   <div className="w-full mt-6 mb-4 bg-black/40 p-5 rounded-xl border border-zinc-800 shadow-inner">
                                     {keys.length > 1 ? (
                                       <p className="text-xs text-zinc-500 mb-3 leading-relaxed">
                                         Each line is a different filing or company (see the legend). The x-axis is an ordered
                                         list of values pulled from the retrieved snippets — use the answer text for what each
                                         point represents.
                                       </p>
                                     ) : null}
                                     <div className="h-72">
                 ...
                 {msg.role === 'assistant' && msg.scoped_sources?.length > 0 && !msg.isSystem && (
                   <div className="px-2 mt-1 text-[11px] text-zinc-500 max-w-[85%] leading-relaxed">
                     <span className="text-zinc-600">Based on </span>
                     <span className="text-zinc-400">{msg.scoped_sources.join(', ')}</span>
                     {msg.auto_scoped ? <span className="text-zinc-600"> (selected for this question)</span> : null}
                   </div>
                 )}
                 ...
                 {msg.role === 'assistant' && !msg.isSystem && msg.evidence?.length > 0 && (
                   <details className="max-w-[85%] mt-2 rounded-lg border border-zinc-700/80 bg-zinc-950/60 text-left group/ev">
                     <summary className="cursor-pointer list-none px-3 py-2 text-xs text-zinc-400 hover:text-emerald-400 flex items-center gap-2 [&::-webkit-details-marker]:hidden">
                       <BookMarked size={14} className="text-emerald-600 shrink-0" />
                       <span>Sources ({msg.evidence.length})</span>
                       <span className="text-zinc-600 truncate">{msg.sources_summary || ''}</span>
                     </summary>
                     <div className="px-3 pb-3 pt-0 space-y-2 border-t border-zinc-800/80">
                       <p className="text-[10px] text-zinc-600 pt-2">
                         Short excerpts from the filings used for this answer. Relevance is approximate.
                       </p>
                       {msg.evidence.map((ev, j) => (
                         ...
                         >
                           <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 text-[11px] text-zinc-300 mb-1.5">
                             <span className="text-emerald-500/90 truncate max-w-[220px]" title={ev.source}>{ev.source}</span>
                             <span className="text-zinc-500">Page {ev.page != null ? ev.page : '—'}</span>
                             <span className="text-zinc-600 tabular-nums">
                               relevance {(ev.confidence_score * 100).toFixed(0)}%
                             </span>
                           </div>
                           <p className="text-zinc-500 leading-relaxed">{ev.excerpt}</p>
                         </div>
                       ))}
                     </div>
                   </details>
      ...
      <div className="absolute bottom-0 w-full p-6 pt-12 bg-gradient-to-t from-[#0c0c0e] via-[#0c0c0e]/90 to-transparent">
         <div className="max-w-4xl mx-auto flex items-center gap-3 bg-zinc-900 border border-zinc-700 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] rounded-xl p-2 px-4 transition-all focus-within:border-emerald-500/50">
            <input 
              ...
              placeholder="Ask about revenue, risks, margins, or compare companies…"
              className="flex-1 bg-transparent text-white focus:outline-none placeholder-zinc-500 text-sm h-11 px-2"
            />
            <button 
              onClick={() => handleSend()} 
